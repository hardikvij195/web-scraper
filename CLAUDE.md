...
> **Versioning (2026-08-26):** `VERSION` (1.0.1, 1.0.2 … 1.0.9 → 1.1.0) — run `python scripts/bump-version.py` before EVERY push, whatever changed (user directive 2026-08-27); the CRM shows "outdated" against main's `VERSION`.
...
| `WA_WINDOW` / `WA_WINDOW__<DEVICE>` | `visible` | W86: WhatsApp verify Chrome: `visible` window, `hidden` (installed Chrome parked off-screen), or `headless` (installed Chrome `--headless=new`). Set per machine on the CRM Systems card. Login (QR) always visible; a non-visible session that never renders falls back to visible |
